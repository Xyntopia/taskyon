...
implementation:
  ...
  - packages/tycli/src/cli/fileStorage.ts
---
...
specified in `encrypted-storage.md`.
...
- Store durable records and blobs as encrypted storage objects with bucketed
  ...
  described in `encrypted-storage.md`.
...
```mermaid
flowchart TD
  Caller[Core and tool code] --> Client[Scoped storage client]
  Client --> Policy[Schema and namespace policy]
  Policy --> Transform[Compression encryption and chunking]
  Transform --> Protocol[Storage protocol over typed ports]
  Protocol --> Service[Selected storage service]
  Service --> Browser[Browser OPFS]
  Service --> Files[CLI files]
  Service --> Cloud[Object store gateway]
  Service --> Peer[P2P gateway]
  Service --> Index[Local rebuildable indexes]
```
...
`encrypted-storage.md`; the runtime storage layer should only
...
```mermaid
flowchart TD
  List[List opaque backend objects] --> Decrypt[Try available namespace keys]
  Decrypt --> Metadata[Read authenticated metadata]
  Metadata --> Group[Group namespace path and version]
  Group --> Verify[Verify chunks and hashes]
  Verify --> Rebuild[Rebuild manifests]
  Rebuild --> Retry[Retry normal lookup]
```
