...
core/tool code
  ...
  -> repository helpers for tasks, DAG nodes, tools, secrets, artifacts
  -> transform layers
       - schema validation
       - compression
       - chunking
       - encryption/signing
       - object naming
  -> storage protocol over FRP/ports
  -> storage service
  -> backend adapter
       - OPFS / IndexedDB
       - CLI files under Taskyon runtime roots
       - memory
       - PGlite cache/index
       - cloud object store
       - P2P gateway
       - Syncthing folder bridge
```
...
list backend objects
  -> try decrypt with available namespace keys
  -> read authenticated plaintext metadata
  -> group by namespace/path/version
  -> verify content hashes and chunk counts
  -> rebuild directory, namespace, latest-version, and blob manifests
```
