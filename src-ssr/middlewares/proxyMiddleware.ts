...
if (!PROXY_JWT_PUBLIC_KEY) {
  throw new Error('Missing PROXY_JWT_PUBLIC_KEY env var')
}
...
async function verifyServiceToken(jwt: string): Promise<ServiceTokenPayload> {
...
const handleProxy = async (req: Request, res: Response) => {
  try {
    ...
    const token = await verifyServiceToken(jwt)
...
export default defineSsrMiddleware(({ app }) => {
  ...
  app.all('/proxy/:service/*', handleProxy)
})
