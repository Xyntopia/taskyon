<template>
    ...
    <div class="col row content-stretch full-height">
      <div class="col-auto col items-center">
        <div class="col-auto">
          ...
          >
            <q-tab name="upload" :icon="matUpload"
              ><q-tooltip :delay="500">open upload window</q-tooltip></q-tab
            >
            <q-tab name="settings" :icon="matSettings"
              ><q-tooltip :delay="500">open store settings</q-tooltip></q-tab
            >
            <q-tab name="whitelist" :icon="matKey"
              ><q-tooltip :delay="500">open access whitelist</q-tooltip></q-tab
            >
          </q-tabs>
        </div>
        <q-separator class="q-my-xs" />
        <div class="col-auto row text-center items-center justify-center">
          <q-icon :name="matLocalLibrary" />:
      ...
      <div class="col">
        ...
        >
          ...
          <q-tab-panel name="settings">
            <q-input
              v-if="addCollection"
              label="Input name for new collection"
              outlined
              dense
            ...
            >
              <template v-slot:after>
                <q-btn
                  ...
                  flat
                  :icon="matDone"
                  @click="onNewCollection(newCollectionName)"
                >
            ...
            >
              ...
              <template v-slot:after>
                <q-btn
                  ...
                  flat
                  :icon="matAddBox"
                  @click="addCollection = true"
                  ><q-tooltip :delay="500"
                    >Create new collection.</q-tooltip
                  ></q-btn
                >
              </template>
            </q-select>
            <div class="text-caption">
              Chunks: {{ vectorStoreState.numElements }}
            </div>
          </q-tab-panel>
          <q-tab-panel name="whitelist">
            <div class="text-bold">
              Webpages who have been granted access to the document store:
            </div>
            <q-virtual-scroll
              bordered
              :items="uploaderState.accessWhiteList"
              separator
              v-slot="{ item, index }"
            >
  ...
  <div v-else class="column q-gutter-sm items-center content-stretch">
    <q-btn
      ...
      @click="grantAccess(parentUrl)"
    >
      <div>Grant search access to:</div>
      <q-tooltip
        >Allow page {{ parentUrl }} to search our vector store</q-tooltip
      >
    </q-btn>
...
function blockAccess(page: string) {
  ...
  uploaderState.value.accessWhiteList =
    uploaderState.value.accessWhiteList.filter((p) => p !== page);
  checkAccess();
}
