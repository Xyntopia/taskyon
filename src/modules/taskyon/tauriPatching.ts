...
const getUniqueContextRange = (
  ...
  for (const strategy of contextMatchStrategies) {
    ...
    if (validRanges.length === 1) {
      return validRanges[0] as MatchRange
    }
...
const assertValidPatch = (patch: SearchReplacePatch, patchIndex: number) => {
  ...
  const contextLines = Math.max(
    MIN_CONTEXT_LINES,
    Number.isFinite(rawContextLines) ? (rawContextLines as number) : MIN_CONTEXT_LINES,
  )
...
const assertNoOverlaps = (matches: Array<MatchRange & { patchIndex: number }>) => {
  ...
  for (let index = 1; index < sorted.length; index += 1) {
    const previous = sorted[index - 1] as (MatchRange & { patchIndex: number }) | undefined
    const current = sorted[index] as (MatchRange & { patchIndex: number }) | undefined
...
export const getFileUpdateMode = (update: FileUpdate): FileUpdateMode => {
  ...
  return modes[0] as FileUpdateMode
}
