...
export async function testArchiveUploadDownload() {
  ...
  const uploaded = await useGdrive(tystate.getToken).uploadFileArchiveWMeta(
    dir,
    fakeZipFile,
    logicalFilenames,
    true,
  ...
  const downloaded = await useGdrive(tystate.getToken).downloadArchiveFile(dir, 'foo.txt')
  results.downloaded = {
    ok: !!downloaded,
...
export async function testGdriveZipRoundtrip() {
  ...
  try {
    ...
    const gdport = gDriveSyncPort(directory, tystate.getToken)
