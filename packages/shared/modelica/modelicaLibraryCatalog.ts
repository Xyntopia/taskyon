...
const libraryZipModules = import.meta.glob<string>('../../../../public/modelica-libraries/*.zip', {
  eager: true,
  import: 'default',
  query: '?url',
})

const rootLibraryZipModules = import.meta.glob<string>('/public/modelica-libraries/*.zip', {
  eager: true,
  import: 'default',
  query: '?url',
})
...
const mergedLibraryZipModules: Record<string, string> = {
  ...libraryZipModules,
  ...rootLibraryZipModules,
}
const manifestLibraries = Object.values(manifestModules)
  .flatMap((entry) => entry.libraries ?? [])
...
const presetUrlById = detectedModelicaLibraryPresets.reduce<Record<string, string>>((acc, preset) => {
  const key = String(preset.id || '').trim().toLowerCase()
  if (!key) return acc
  acc[key] = preset.url
  return acc
}, {})
...
console.info(
  '[modelica-library-catalog] root glob matches:',
  Object.keys(rootLibraryZipModules),
)
console.info(
