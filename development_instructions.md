...
- Do not run automatic lint error correction by default during every turn. If the user explicitly asks for lint cleanup, or if you are preparing a final pre-commit verification pass, you may run `yarn lint:fix -- <path ...>` (or the package-local equivalent) only for the files you changed or created.
...
- Do not run broad repo-wide lint commands by default at the end of every task. Prefer targeted verification for the changed boundary, and remind the user to run the full lint pass before committing when that broader check was not run in the turn.
